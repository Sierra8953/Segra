...
bool ObsManager::Initialize() {
    ...
    // Assumptions for path layout (deployed via CMake)
    ...
    // Configure Default Video (1080p60)
...
bool ObsManager::StartRecording(const std::string& path) {
    if (!is_initialized) return false;

    // FFmpeg Muxer Settings for Fragmented MP4
    obs_data_t* settings = obs_data_create();
    obs_data_set_string(settings, "path", path.c_str());
    obs_data_set_string(settings, "format_name", "mp4");
    // Critical flags for "flawless" resilience and live preview
    ...
    // Encoder (x264 software fallback for compatibility, can upgrade to NVENC)
    ...
    // Setup Basic Scene (Display Capture)
    // NOTE: For a real app, this should be more dynamic.
    // Here we try to create a display capture source.
    obs_data_t* display_settings = obs_data_create();
    obs_data_set_int(display_settings, "monitor", 0);
    obs_source_t* source = obs_source_create("monitor_capture", "display", display_settings, nullptr);
